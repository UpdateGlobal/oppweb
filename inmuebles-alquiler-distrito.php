...
<body>
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-8 col-xs-12">
        <p class="branch">Inicio > Alquiler > <?php echo $xDistrito; ?></p>
      </div>
      <!-- 
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-8">
        <?php
          $consultarInm = "SELECT * FROM inmuebles WHERE cod_distrito='$cod_distrito' AND alquiler='1' AND estado='1'";
        ...
        <?php
            ...
            $consultarInm = "SELECT ci.cod_categoria, ci.categoria, li.cod_lugar, li.lugar, di.cod_distrito, di.distrito, i.* FROM inmuebles_categorias as ci, inmuebles_lugares as li, inmuebles_distritos as di, inmuebles as i WHERE i.cod_categoria=ci.cod_categoria AND i.cod_lugar=li.cod_lugar AND i.cod_distrito=di.cod_distrito AND i.cod_distrito='$cod_distrito' AND alquiler='1' ORDER BY orden ASC $limite";
            $resultadoInm = mysqli_query($enlaces, $consultarInm) or die('Consulta fallida: ' . mysqli_error($enlaces));
        ...
        if ($total_paginas>1){
          echo "<div class='row text-center'>
                  <div class='col-lg-12'>
                    <ul class='pagination'>";
                  if($pagina>1){
                    echo "<li><a href='/alquiler-distritos/".$slug."&p=".($pagina-1)."'>&laquo;</a></li>";
                  }
                  for($i=$pagina; $i<=$total_paginas && $i<=($pagina+$paginas_mostrar); $i++){
                    if($i==$pagina){
                      echo "<li class='active'><a><strong>$i</strong></a></li>";
                    }else{
                      echo "<li><a href='/alquiler-distritos/".$slug."&p=$i'>$i</a></li>";
                    }
                  }
                  ...
                  if($pagina<$total_paginas){
                    echo "  <li><a href='/alquiler-distritos/".$slug."&p=".($pagina+1)."'>&raquo;</a></li>";
                  }
        echo "  </ul>
              </div>
            </div>";
